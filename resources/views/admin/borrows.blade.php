...
@section('title', 'Peminjaman - Admin Bookify')
...
<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;margin-bottom:20px;">
    <div style="background:#fff3cd;border-radius:12px;padding:16px 20px;box-shadow:0 2px 8px rgba(0,0,0,0.07);">
    ...
    <div style="background:#dbeafe;border-radius:12px;padding:16px 20px;box-shadow:0 2px 8px rgba(0,0,0,0.07);">
...
<div class="card-box">
    <div class="card-title">Semua Data Peminjaman ({{ $borrows->count() }} data)</div>
    <div style="overflow-x:auto;">
        <table class="tbl">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($borrow->status === 'menunggu')
                            <form action="{{ route('admin.borrows.approve', $borrow) }}" method="POST" style="display:inline;" onsubmit="return confirm('Setujui peminjaman ini?')">
                                @csrf
                                <button type="submit" class="btn-success" style="padding:5px 12px;font-size:12px;border-radius:6px;border:none;cursor:pointer;font-family:inherit;">✓ Setujui</button>
                            </form>
                            <button onclick="showRejectModal({{ $borrow->id }}, 'admin')" class="btn-danger" style="padding:5px 12px;font-size:12px;border-radius:6px;border:none;cursor:pointer;font-family:inherit;margin-left:4px;">✗ Tolak</button>

                        @elseif($borrow->status === 'menunggu_kembali')
                            <form action="{{ route('admin.borrows.confirm.return', $borrow) }}" method="POST" style="display:inline;" onsubmit="return confirm('Konfirmasi pengembalian ini?')">
                                @csrf
                                <button type="submit" class="btn-success" style="padding:5px 12px;font-size:12px;border-radius:6px;border:none;cursor:pointer;font-family:inherit;">✓ Konfirmasi Kembali</button>
                            </form>
                ...
                @empty
                <tr><td colspan="8" style="text-align:center;color:#aaa;padding:30px;">Belum ada data peminjaman.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
function showRejectModal(id, role) {
    var base = role === 'admin' ? '/admin/borrows/' : '/petugas/borrows/';
    document.getElementById('reject-form').action = base + id + '/reject';
